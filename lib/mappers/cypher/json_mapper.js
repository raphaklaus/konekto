...
function cypherNodeToMergeStatement (idGenerator, cypherNode) {
  ...
  return new Statement(
    `MERGE (${cypherNode.id}:${cypherNode.label} {uuid: $${uuidParam}}) ${onMatchCypher(cypherNode.id)}`,
    {
...
function getRelationshipStatements (idGenerator, parentCypherNode, children, statements = []) {
  for (let item of children) {
    ...
    let statement = parentChildToMergeStatement(idGenerator, parentCypherNode, item.key, cypherNode, item.isArray)
